...
import { z } from "zod";
import { Bot, Workflow, MessageSquare, CheckCircle2, ChevronRight } from "lucide-react";
...
const formSchema = z.object({
  ...
  if (data.enquiryType === "General enquiry") {
    if (!data.message || data.message.trim().length < 10)
      ctx.addIssue({ code: "custom", path: ["message"], message: "Please tell us a little more" });
  }
});
type FormData = z.infer<typeof formSchema>;

type ArrayFieldName = "channels" | "chatbotGoals" | "tasksToAutomate";
type SelectFieldName = "enquiryVolume" | "toneOfVoice" | "hoursPerWeek" | "timeline";
...
function normaliseType(raw: string): string {
  ...
  if (v.includes("general")) return "General enquiry";
  return "";
}
                ...
                <FormItem>
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent className="bg-card border-white/10">
                      ...
                      <SelectItem value="General enquiry" className="text-white focus:bg-primary/20 focus:text-white cursor-pointer">
                        General enquiry — a question or something else
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage className="text-red-400" />
            ...
            {enquiryType && (
              <div className="mt-4 flex items-center gap-3 text-sm text-muted-foreground">
                {enquiryType === "AI Chatbot" && (
                  <><Bot className="w-4 h-4 text-primary shrink-0" /> A smart assistant that talks to your customers across web &amp; social.</>
                )}
                {enquiryType === "AI Workflow" && (
                  <><Workflow className="w-4 h-4 text-primary shrink-0" /> Hands-off automations that do the repetitive admin for you.</>
                )}
                {enquiryType === "General enquiry" && (
                  <><MessageSquare className="w-4 h-4 text-primary shrink-0" /> A general question or something else entirely — we're all ears.</>
                )}
              </div>
            )}
          </div>
            ...
            <>
              ...
              {enquiryType === "General enquiry" && (
                <div className="space-y-6">
                  <div>
                    <h2 className="text-lg font-bold text-white mb-2">Your enquiry</h2>
                    <p className="text-sm text-muted-foreground">Tell us what's on your mind and we'll point you in the right direction.</p>
                  </div>
                  <FormField control={form.control} name="message" render={({ field }) => (
                    <FormItem><FormLabel className="text-white/70">How can we help?</FormLabel><FormControl><Textarea className="bg-background border-white/10 text-white rounded-xl min-h-[160px] resize-none p-4" placeholder="Type your question or message here…" {...field} /></FormControl><FormMessage className="text-red-400" /></FormItem>
                  )} />
              ...
              {/* Common closing for AI branches */}
              {(enquiryType === "AI Chatbot" || enquiryType === "AI Workflow") && (
                <>
                  <hr className="border-white/[0.04]" />
